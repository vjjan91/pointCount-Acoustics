...
# Detection ratios (point counts-acoustics) vs. vocalization duration

In this script, we will explore relationships between the ratio of bird detections in a point count and acoustic survey and the median vocalization duration of a species. To do this, we will continue to rely on detections estimated at the site-level (similar to what was required for the indicator species analysis). The goal here is to ask if species with relatively higher detections in one method or the other have shorter vocalization durations? 
...
## Exploring ratios of detections in point counts & acoustic surveys to median delta time of a species  
```{r}
...
# join the time data to species trait dat
dat_median <- left_join(median_time_10, trait, by = "eBird_codes")
...
ggsave(fig_detec_duration, filename = "figs/fig_medianDeltaTime_detectionRatio.png", width = 14, height = 16, device = png(), units = "in", dpi = 300)
dev.off() 
```

![Marginal associations - no real support seen.](figs/fig_medianDeltaTime_detectionRatio.png)  
